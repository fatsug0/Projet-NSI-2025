...
public class WeaponGunScript : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        ammoInReserve = (int)stats.weaponMagazineSize * 3;
    }
    ...
    private void Update()
    {
        ...
        if (allowContinuousShots)
        {
            // Shoot continuous bullets if it has the bullets
            if (shootAction.action.IsPressed() && currentNumberOfBullet > 0 && !_isReloading && !_isShooting) 
            {
        ...
        {
            // Shoot a single bullet if it has the bullets
    ...
    private IEnumerator Shoot()
    {
        ...
        // if (singleShot)
        // {
        //     // Apply no shoot spread IF ALLOWED !
        // }
        // else
        // {
        // Apply shoot spread
        float spreadAmount = (int)stats.weaponShootSpread * quaternionCoef; // Tweak this multiplier to adjust spread sharpness
